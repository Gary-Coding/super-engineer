...
- `/se:propose` 阶段优先调用 OpenSpec CLI 创建 change。
...
OpenSpec change 名称默认从 `vars.demand_name` 推导。如果需求名以数字前缀开头，例如：

```text
7-deamnd-addition-rate
```

实际 OpenSpec change 名称会归一为：

```text
deamnd-addition-rate
```

这样可以兼容团队用数字给需求目录排序的习惯，同时避免 OpenSpec change 名称以数字开头。
...
6. 用户审核 `todo.md` 后发 `/se:approve`。
7. 用户发 `/se:apply`。
8. AI 调用 `run-workflow.py plan`，生成 session、discovery、plan。
9. AI 根据 `plan.json` 修改业务代码。
10. AI 调用 `finish-implement` 生成 self-check。
11. AI 调用 `review` 生成 review 门禁。
12. AI 调用 `verify` 执行验证命令。
13. OpenSpec 模式下自动回写 execution summary。
...
2. 增强 approval 机制：把 `/se:approve` 的审核标记、审核人、todo hash 固化。
