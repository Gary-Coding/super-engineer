...
这里最重要的一点是：  
不要直接从自动实现开始。应该先从 OpenSpec change 开始，产出 `tasks.md`，再桥接成 `todo.generated.md`，审核通过后再进入自动交付。
...
/se:propose
需求是：经销商用户列表接口增加手机号精确筛选，要求兼容旧查询行为，并补齐 controller / service 层测试。
请为当前需求生成或完善 proposal、design 和 tasks。
```
...
```text
/se:bridge
针对当前 OpenSpec change 生成 todo.generated.md。
...
/se:propose
需求是：经销商用户列表接口增加手机号精确筛选，要求兼容旧查询行为，并补齐 controller / service 层测试。
请生成当前 change 的 proposal、design 和 tasks。
```
...
```text
/se:bridge
针对当前 OpenSpec change 生成 todo.generated.md，并列出待审核项。
```
...
```text
/se:approve
我已审核当前 todo.generated.md，可以开始交付阶段。
```
...
请补充当前 change 的 proposal、design 和 tasks，不要创建新的 change。
```
...
```text
/se:bridge
针对更新后的 OpenSpec change 重新生成 todo.generated.md，并列出新增或变化的待审核项。
```
...
```text
/se:approve
我已审核更新后的 todo.generated.md，可以继续交付遗漏修复。
```
...
/se:apply
继续当前工作空间。
按已审核的 todo.generated.md 修复遗漏，并完成自查、审查和验证。
```
...
已归档需求发现后续遗漏：
手机号筛选需要同时兼容脱敏手机号字段，否则部分历史用户无法被查到。
请创建一个 follow-up change，并生成 proposal、design 和 tasks。
```
...
- `/se:propose`：生成或完善 OpenSpec change
- `/se:bridge`：把 `tasks.md` 转成 `todo.generated.md`
