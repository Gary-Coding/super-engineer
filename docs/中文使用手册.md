...
| 3 | `/se:bridge` | 把 OpenSpec `tasks.md` 桥接成 `todo.md` | 人工审核 todo.md 后 `/se:approve` |
| 4 | `/se:approve` | 记录桥接 todo 已人工审核通过 | `/se:plan` 或 `/se:apply` |
| 5 | `/se:plan` | 生成交付计划，不改代码 | `/se:apply` |
| 6 | `/se:apply` | 进入实现、自查、审查、验证 | `/se:archive-check` |
| 7 | `/se:review` | 单独执行或重跑代码审查 | `/se:verify` 或 `/se:apply` 修复 |
| 8 | `/se:verify` | 单独执行或重跑验证 | 通过后 `/se:archive-check` |
| 9 | `/se:archive-check` | 检查是否满足 OpenSpec 归档条件 | 满足 safe_merge 后 `/se:archive` |
| 10 | `/se:archive` | 执行 OpenSpec 归档 | 结束 |
...
- `/se:approve` 只记录审核通过，不能自动开始实现
- `/se:apply` 之前必须已经完成 `/se:bridge` 和 `/se:approve`
